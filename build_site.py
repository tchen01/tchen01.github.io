...
for folder in folders:
    ...
    for file in md_files:
        ...
        if file[:-3]+'.html' in html_files:
            ...
            # convert md to html
            os.system(f'pandoc -o {folder}/{file[:-3]}1.html {folder}/{file}')
            
            # generate updated pdf
            os.system(f'pandoc -o {folder}/{file[:-3]}.pdf {folder}/{file}')
            
            # open old html file, pandoc converted html file, and temp new file
            with open(f'{folder}/{file[:-3]}.html','r') as old_html_file, open(f'{folder}/{file[:-3]}1.html','r') as new_html_content, open(f'{folder}/{file[:-3]}2.html','w+') as new_html_file:
            ...
            os.system(f'rm {folder}/{file[:-3]}1.html')
            os.system(f'mv {folder}/{file[:-3]}2.html {folder}/{file[:-3]}.html')
